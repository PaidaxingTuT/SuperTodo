...
| v1.6.5 | 开源仓库按钮改主色实心胶囊；尝试固定签名（实测未生效，见 SIGNING_REPORT.md）；原生下载 + ghfast 加速 |
| v1.6.6 | 简化分类文案；调整仓库按钮尺寸；增强 AI 速记解析并支持关闭时取消请求 |

## 九、现状与注意事项

- Git：工作树干净，与 `origin/main` 同步；tag 已到 `v1.6.6`
- ⚠️ 已发布 v1.6.5 / v1.6.6 实为随机签名（固定签名在 CI 未生效，详见 SIGNING_REPORT.md）。**自 v1.6.7 显式签名起才真正同钥**；切换需最后一次卸载重装（先导出备份）
